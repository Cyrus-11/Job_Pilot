...
### Homepage additions — 2026-09-09

The supplied landing-page design uses dark CTA buttons and pale gradient sections. These semantic tokens are defined in `app/globals.css` for that page:

| Token | Value | Purpose |
| --- | --- | --- |
| `--color-landing-heading` | `#241b2e` | Hero headline and secondary CTA text |
| `--color-landing-button` | `#252430` | Dark primary CTA |
| `--color-landing-button-top` | `#42424e` | Primary CTA gradient top |
| `--color-landing-glow-blue` | `#c9ddf1` | Hero and closing CTA blue glow |
| `--color-landing-glow-pink` | `#f4e7f3` | Hero and closing CTA pink glow |

Inter now uses the Next font variable `--font-inter`; `@theme inline` maps `--font-sans` to `var(--font-inter)`. This avoids defining the font variable twice while preserving the `font-sans` utility.
